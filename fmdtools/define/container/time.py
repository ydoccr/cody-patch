...
class Time(BaseContainer):
    ...
    t_ind: np.int64 = np.int64(0)
    timers: dict = {}
    use_local: np.bool_ = np.bool_(True)
    dt: np.float64 = np.float64(1.0)
    executed_static: np.bool_ = np.bool_(False)
    executed_dynamic: np.bool_ = np.bool_(False)
    executing: np.bool_ = np.bool_(False)
